...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        while len(eliminationOrder) > 0:
            join = eliminationOrder[0]

            currentFactorsList, new_factor = joinFactorsByVariable(currentFactorsList, join)

            if len(new_factor.unconditionedVariables()) == 1:
                eliminationOrder.remove(join)
                continue

            currentFactorsList.append(eliminate(new_factor, join))
            eliminationOrder.remove(join)
