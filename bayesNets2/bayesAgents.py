...
def constructBayesNet(gameState):
    ...
    "*** YOUR CODE HERE ***"
    position =[position for pos in gameState.getPossibleHouses() for position in gameState.getHouseWalls(pos)]

    for var2 in position:
        obsVar = OBS_VAR_TEMPLATE % var2
...
def fillObsCPT(bayesNet, gameState):
    ...
    for wall in position:
        ...
        for assignment in obsFactor.getAllPossibleAssignmentDicts():
            blue, none , red = 0,0,0
            if assignment[FOOD_HOUSE_VAR] == getHouseValCorrespToPos(wall[0], wall[1]):
                blue,none,red = 1-PROB_FOOD_RED,0,PROB_FOOD_RED
            elif assignment[GHOST_HOUSE_VAR] == getHouseValCorrespToPos(wall[0], wall[1]):
                blue,none,red = 1-PROB_GHOST_RED,0,PROB_GHOST_RED
            else:
                blue,none,red=0,1,0
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    variable = inference.inferenceByVariableElimination(bayesNet, ['foodHouse', 'ghostHouse'], evidence, eliminationOrder)
    max = float('-inf')
    list = util.Counter()

    dictionary = [assign for assign in variable.getAllPossibleAssignmentDicts()]

    for assignment in dictionary:
        if variable.getProbability(assignment) > max:
            max = variable.getProbability(assignment)
            list = assignment
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        variable = inference.inferenceByVariableElimination(self.bayesNet, ['foodHouse', 'ghostHouse'], evidence, eliminationOrder)
        topleft,topright = 0.0, 0.0

        for assignment in variable.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                topleft += variable.getProbability(assignment)
            elif assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                topright += variable.getProbability(assignment)

        leftExpectedValue = topleft * WON_GAME_REWARD + topright * GHOST_COLLISION_REWARD
        rightExpectedValue = topright * WON_GAME_REWARD + topleft * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for variables in self.getExplorationProbsAndOutcomes(evidence):
            prob , outcome = variables
            evidence.update(outcome)
            val1, val2 = self.computeEnterValues(evidence, enterEliminationOrder)
            expectedValue += max(val1, val2) * prob
