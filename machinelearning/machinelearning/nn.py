...
class Graph(object):
    ...
    def get_gradient(self, node):
        ...
        return self.gradients.get(node)
    ...
    def add(self, node):
        ...
        node_list = [self.get_output(x) for x in node.get_parents()]

        self.inputs[node] = node_list
        self.outputs[node] = node.forward(self.get_inputs(node))
        self.gradients[node] = np.zeros_like(node.forward(self.get_inputs(node)))
    ...
    def backprop(self):
        ...
        """
        loss = self.get_nodes()[-1]
        assert np.asarray(self.get_output(loss)).ndim == 0

        self.gradients[loss] = 1.0
        for node in self.nodes[::-1]:
            back = node.backward(self.get_inputs(node), self.get_gradient(node))
            node_parents = node.get_parents()

            for p_index in range(0, len(node_parents)):
                self.gradients[node_parents[p_index]] +=  back[p_index]
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [np.matmul(gradient, np.transpose(inputs[1])) , np.matmul(np.transpose(inputs[0]), gradient)]
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        inp[inp < 0] = 0
        inp[inp>0] = 1
        return [np.multiply(gradient, inp)]
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        return np.average(np.square(inputs[0]-inputs[1])*.5)


    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [(inputs[0]-inputs[1])*gradient/inputs[0].size, -1 *gradient * (inputs[0]-inputs[1])/inputs[0].size]
